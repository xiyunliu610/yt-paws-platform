...
2. **External call (if `stripe`):** `stripe.refunds.create({ payment_intent }, { idempotencyKey: 'refund_' + payment.id })` against the `paymentIntentId` captured on the succeeded `StripeCheckoutAttempt` (see §6.1's amendment below). The idempotency key means a client-side retry of the same refund request can't double-refund the same charge. `wechat_qr` skips this step entirely — no external API to call; the owner marking a payment refunded is them asserting they've already returned the money manually outside the app, the same trust model `verifyWechatPayment` already uses for confirming a transfer was *received*. The frontend button is explicitly labeled "Mark Manual Refund as Completed" for this method, with a confirmation step, specifically so it can't be mistaken for the Stripe path's actual automatic refund.
3. **Finalize:** on success, `refund_pending → refunded` (storing `stripeRefundId` for Stripe payments); on failure, `refund_pending → paid` — safe from colliding with a new payment, since `refund_pending` was itself covered by the unique index the whole time.

The customer is notified (`'Payment Refunded'`) once step 3 completes as `refunded`.

**Known gap:** if the process crashes between Stripe confirming the refund (step 2) and the `refunded` write landing (step 3), the payment is stuck in `refund_pending` with the money already returned by Stripe — there's no automatic reconciliation for that yet (would need a Stripe refund webhook, or a periodic job querying Stripe for anything stuck in `refund_pending`). This is a manual fix-up for now (check the Stripe dashboard, then correct the row directly), not solved end-to-end. It's a narrower window than the bug this revision fixed — a crash exactly between "Stripe confirms" and "one more DB write" — but it is real.
