...
### 6.3 Refund Flow (added 2026-07-31)
...
- **Ordering matters:** the transition `paid → refunded` is claimed atomically (`updateMany({ where: { status: 'paid' } })`) *before* any external call, not after — so only the request that wins the claim ever calls the Stripe refund API, and two concurrent refund attempts can't both issue one. If the booking's Payment is `stripe`, `stripe.refunds.create({ payment_intent })` is called against the `paymentIntentId` captured on the succeeded `StripeCheckoutAttempt` (see §6.1's amendment below); if that call fails, the claim is rolled back to `paid` — the payment is, in reality, still paid — rather than being left `refunded` with no actual refund having happened.
- `wechat_qr` has no external API to call: the owner marking a payment refunded is them asserting they've already returned the money manually outside the app, the same trust model `verifyWechatPayment` already uses for confirming a transfer was *received*.
- The customer is notified (`'Payment Refunded'`) once the refund (Stripe call, or the WeChat manual confirmation) succeeds.
