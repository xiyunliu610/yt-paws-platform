...
export function validateProductionConfig(env: NodeJS.ProcessEnv) {
  ...
  const missing = REQUIRED_PRODUCTION_VARIABLES.filter((key) => !env[key]?.trim());
  if (missing.length) throw new Error(`Missing required production configuration: ${missing.join(', ')}`);
  if (!env.PUBLIC_WEB_URL!.startsWith('https://')) throw new Error('PUBLIC_WEB_URL must use HTTPS in production');
