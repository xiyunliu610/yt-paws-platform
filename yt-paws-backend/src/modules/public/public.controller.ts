...
export class PublicController {
  ...
  privacy(@Res() res: Response) {
    res.type('html').send(shell('Privacy Policy', `<h1>Privacy Policy</h1><p>Last updated: 1 August 2026.</p><h2>Data we process</h2><p>We process account contact details, pet profiles and health notes, booking and care reports, photos, notification tokens, and payment status/provider references to operate Y&amp;T Paws.</p><h2>Storage and sharing</h2><p>Images are stored in configured cloud object storage under random object keys. Payment card details are handled by Stripe and are not stored by Y&amp;T Paws. We share data only with service providers needed to run the service or where legally required.</p><h2>Deletion and retention</h2><p>Account deletion anonymises the account, removes pet care details, health records, report text/photos and notifications. Minimal booking, service-price snapshot and payment records are retained for accounting, refunds, fraud prevention and disputes. See the <a href="/account-deletion">account deletion page</a>.</p><h2>Your choices</h2><p>You may request access, correction or deletion using the App or the external request page.</p>`));
  }
